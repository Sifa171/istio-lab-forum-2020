...
[source,bash]
----
xdg-open https://$(oc get route -n user1-smcp kiali -o 'jsonpath={.spec.host}')
----

When prompted to login use the username *{{ LABUSER }}* and the password *{{ LABUSER_PASSWORD }}*.
...
* Namespace: user1-tutorial
* Service: customer.user1-tutorial
